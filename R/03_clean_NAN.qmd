...
dataset <- read_csv(data_path,na = c( "NA","nd","-","na","n"), show_col_types = FALSE)
problems(dataset)
head(dataset)
```
...
The subjectID are also duplicated (1805 duplicates).
...
-   hmp / hmpii: mentioned in the paper, but number of samples unspecified
...
We can see that the vast majority of metadata columns completely empty, so we can remove them.
