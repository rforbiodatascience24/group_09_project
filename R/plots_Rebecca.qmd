...
### Vitulazation of the perammeteres

First, we want to see the distribution of BMI classes. In the plot below, we observe that there are slightly more overweight people in the dataset.
...
Then we want to see the distribution of glucose tolerance in the dataset. In the plot below, we observe that we have roughly the same distribution between the three groups.
...
dataset |> 
  mutate(s_Faecalibacterium_prausnitzii_group = cut(
    ...
    breaks = seq(from = 0.000,
                 ...
                 by = 5),
    include.lowest = TRUE)) |> 
  count(glucose_tolerance, s_Faecalibacterium_prausnitzii_group) |> 
  ...
  geom_hline(yintercept = 0) +
  theme_minimal(base_size = 10) +
  labs(x = "s_Faecalibacterium_prausnitzii",
       y = "Count",
       title = "s_Faecalibacterium_prausnitzii count group by glucose tolerance",
       fill = "Glucose tollerance: ")+
  theme(legend.position = "bottom",
        panel.grid.major.x = element_blank(),
        axis.text.x = element_text(vjust = 5))
...
```{r}
dataset |> 
  ggplot(mapping = aes(x = wc, y = fasting_insulin, colour = glucose_tolerance )) +
  geom_point()
```
