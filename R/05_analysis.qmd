...
```{r}
dataset |> 
  table1::table1(x = formula(~ BMI_class  + hdl_to_ldl_ratio + `gad-antibodies`+ cholesterol + creatinine| glucose_tollerance),
         data = _)
```
...
```{r}
dataset_dia_estimates1 <- dataset_dia_estimates |> 
