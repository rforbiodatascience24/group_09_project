...
These columns can be really important for analysis of single datasets, but are only really applicable to their own, making them insignificant for comparing the different datasets.

# Decision point

Because of what was said before there are two possible approaches:

1.  Keep all the datasets and only mantain the columns which are significant for most of them. This makes it difficult to perform significant analysis on the metadata as only the informations that were collected in all datasets . On the other hand we have much more data to study in terms of microbiota.

2.  Choose only a single dataset and focus the analysis on that one. This allows us to perform more significant analysis on the metadata, which opens to many of the approaches we studied in the course, and also makes the abundance data smaller, thus making it easier to work with it.

We decided to go for the second option as it allows to better show our competences.

### Approach 1 - We keep all datasets

Even if we are not going to follow this path we still provide an example of how the cleaning of this larger dataset could be done.

```{r}
#Filter if less than 70% of the values in the column are valid
dataset_specific_columns <- valid_counts |> 
  filter(valid_count < nrow(metadata)*0.50) |> 
  pull(column_name)
...
We can see that is the hmpii dataset to not have the country registered, this can be solved as the HMP project only took into account samples from citizens of the US, as can be seen by the human microbiome project web page.

A check allow us to see that the metadata column are now greatly reduced.
...
The `gender` and `country` columns have NAs, but we did not remove as they are really important for basic knowledge of the sample origin and some missing values can still be acceptable.

For the `bmi` it is more up to what the future analysis will focus on,

The last cleaning we do is simply to remove data which do not provide information about the subject, either have the same value for each entry or simply do not give useful information for analysis.
...
### Approach 2 - Focus on a subset

To the purpose of the project we decided that focusing our effort in the analysis of only a dataset was better, due to the interesting metadata columns we focused on the WT2D dataset.
...
Lastly we merge the metadata with the abundance data.

```{r}
# #We can do it without checking the row corrispondance, as we did not remove any row or alter their order during the cleaning.
# dataset <- metadata |> 
#   bind_cols(abundance_data)
```

```{r}
dataset <- inner_join(metadata,
                      abundance_data,
                      by = "sampleID")
dim(dataset)
```

We can see that as expected (as we did not do any row filtering) we don't lose any sample.
