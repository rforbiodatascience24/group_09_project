...
The first thing we do is unify the original datasets that were splitted.
...
```{r}
# Calculate the percentage of valid datapoint that each dataset has in each column
valid_percentage_by_dataset <- metadata |> 
  group_by(dataset_name) |> 
  summarise(across(everything(),
                   ~ sum(!is.na(.)) / n() * 100,  # Original calculation
                   .names = "{col}"))

# Reshape the data for better visualization
valid_percentage_long <- valid_percentage_by_dataset |> 
  pivot_longer(cols = -dataset_name,
               names_to = "column",
               values_to = "valid_percent")

# for each column select the most represented dataset (% > 90%, otherwise) if there is none the the column is considered mixed
most_represented_dataset <- valid_percentage_long |> 
  group_by(column) |> 
  arrange(desc(valid_percent))  |>  
  slice(1) |>   
  ungroup() |> 
  mutate(most_represented_dataset = dataset_name) |> 
  select(-dataset_name)
...
# Assign to each column its the most represented dataset 
columns_with_valid_counts_and_best_dataset <- valid_values_count |> 
  left_join(most_represented_dataset, 
            by = "column")

# Divide the columns based on the amount of NAs they contain 
valid_count_distribution <- columns_with_valid_counts_and_best_dataset |> 
  group_by(valid_count,
           most_represented_dataset) |> 
  summarise(count = n(),
            .groups = "drop")
...
ggplot(aes(x = as.factor(valid_count), y = count, fill = most_represented_dataset)) +
...
ggsave(
  filename = "columns_by_non_NA.jpeg",  
  device = "jpg",                     
  path = "../results",              
  width = 8,                          
  height = 6,                         
...
valid_counts |>
  filter(valid_count != 0) |> 
  ggplot(aes(x = as.factor(valid_count), y = ..count..)) +
  geom_bar(fill = "grey", color = "black", width = 0.8) + # Add colors and border
  coord_flip() +
  labs(
    title = "Columns by number of non-NA values",
    x = "Number of non-NA values",
    y = "Number of columns",
    caption = "The columns with only missing values (125) were omitted for visualization purposes"
  ) +
