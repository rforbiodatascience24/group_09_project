...
In this file we check if the dataset is alreayd present, if it is not we collect it programmatically from kaggle and save it in the \_raw folder.
...
if (already_present) {
  ...
  response <- GET(
    ...
    add_headers(Authorization = paste("Bearer", fromJSON("../kaggle.json")$key)),
...
# data <- read.csv(str_c(target_dir, wanted_file))
