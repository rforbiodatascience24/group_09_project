...
First, we want to get an overview of the metadata present in the dataset, before we analyze it. In the table below, we find parameter values distributed based on glucose tolerance. We see here that there is only one underweight person; therefore, when we look at how values are distributed according to BMI class, we choose not to include it as it does not provide enough data information.

```{r}
dataset |> 
  table1::table1(x = formula(~ BMI_class  +  hdl + cholesterol + creatinine + hdl + wc + whr + statins + insulin + bmi  | glucose_tolerance),
         data = _)
```
...
In the literature, it is stated that BMI can be used as an indicator of type 2 diabetes. Despite this, in our analysis using alpha = 0.05, we did not find the relationship to be statistically significant. Even though BMI was not a statistically significant parameter, we observed that the BMI for those with normal and impaired glucose tolerance is slightly more left-skewed.
...
dataset |> 
  ...
  labs(x = "Fasting insulin",
       y = "Count",
       title = "bmi count by glucose tollerance",
       fill = "Glucose tollerance: ")+
  theme(legend.position = "bottom",
        panel.grid.major.x = element_blank(),
        axis.text.x = element_text(vjust = 5))
...
We investigate how the HDL distribution differs between the glucose tolerance groups, where we see that for the type 2 diabetic group, the distribution is slightly left-skewed.
